  ...
  stages {
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          images=`docker images | grep entropypool | grep inspire-manager | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep inspire-manager | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/inspire-manager/k8s/02-inspire-manager.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/inspire-manager:latest/inspire-manager:$tag/g" cmd/inspire-manager/k8s/02-inspire-manager.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/inspire-manager/k8s/02-inspire-manager.yaml
